...
Progress* Progress::create(CCSpriteFrame* pImg, CCSpriteFrame* pBack, CCRect& rectInsets)
{
	Progress *pRet = new Progress();
	if (pRet && pRet->init(pImg, pBack, rectInsets))
	{
		pRet->autorelease();
		return pRet;
	}
...
void Progress::setProgress(float fProgress)
{
...
bool Progress::init(CCSpriteFrame* pImg, CCSpriteFrame* pBack, CCRect& rectInsets)
{
	bool bRet = false;
    do 
    {
		//CCSpriteFrame* pImage = ImgMgr::getSingleton().getFrame(strImage);
		m_pImage = CCScale9Sprite::createWithSpriteFrame(pImg, rectInsets);
		m_pImage->setAnchorPoint(ccp(0, 0));

		m_pBack = CCSprite::createWithSpriteFrame(pBack);//ImgMgr::getSingleton().loadImg(strBack);
